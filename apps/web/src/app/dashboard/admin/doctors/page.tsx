  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [{ data: me }, { data }] = await Promise.all([authApi.me(), adminApi.listDoctors()]);
      ...
      <header className="sticky top-0 z-30 pl-16 lg:pl-8 pr-8 py-5 glass border-b border-border/50">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="font-display text-xl font-bold text-ink">{th ? "แพทย์ทั้งหมด" : "All Doctors"}</h1>
      ...
      <div className="relative z-10 p-8 space-y-5">
        ...
        {rows.length === 0 ? (
          <div className="glass rounded-2xl py-20 px-8 text-center flex flex-col items-center gap-6">
            <MascotBot variant="idle" size="lg" message={th ? "ยังไม่มีแพทย์ในระบบ" : "No doctors yet"} />
            <p className="font-display font-bold text-ink text-xl">
              {th ? "ยังไม่มีแพทย์ลงทะเบียน" : "No doctors registered yet"}
            </p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="glass rounded-2xl p-16 text-center">
            <p className="font-display font-bold text-ink text-base">{th ? "ไม่พบแพทย์" : "No doctors found"}</p>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            <div className="overflow-x-auto">
              <div className="hidden lg:grid grid-cols-[auto_1fr_140px_120px_120px] gap-4 px-6 py-3 bg-bg2/40 border-b border-border/50 text-[11px] font-semibold text-muted uppercase tracking-wide">
                <span>#</span>
                ...
                <div className="divide-y divide-border/40">
                  {filtered.map((row, i) => {
                    ...
                    return (
                      <div key={row.id}>
                        {/* Desktop row */}
                        <div className="hidden lg:grid grid-cols-[auto_1fr_140px_120px_120px] gap-4 items-center px-6 py-3.5 hover:bg-primary/[0.04] transition-colors duration-150">
