...
const STATUS_STYLE: Record<AdminScanRow["status"], { cls: string; th: string; en: string }> = {
...
export default function AdminScansPage() {
  ...
  const filtered = rows.filter(r => {
    ...
    const matchSearch =
      (r.childName ?? "").toLowerCase().includes(q) || (r.doctorName ?? "").toLowerCase().includes(q);
      ...
      <header className="sticky top-0 z-30 pl-16 lg:pl-8 pr-8 py-5 glass border-b border-border/50">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="font-display text-xl font-bold text-ink">{th ? "การสแกนทั้งหมด" : "All Scans"}</h1>
            <p className="text-xs text-muted mt-0.5">{rows.length} {th ? "รายการทั้งหมด" : "total"}</p>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 p-8 space-y-5">
        <div className="glass rounded-2xl px-4 py-3 flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[180px] group">
            ...
            <input
              type="text"
              placeholder={th ? "ค้นหาชื่อเด็กหรือแพทย์..." : "Search child or doctor..."}
              value={search}
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-bg2/60 border border-border/50 flex-wrap">
            {([
              ["all", th ? "ทั้งหมด" : "All"],
              ["pending", th ? "รอประมวลผล" : "Pending"],
              ["processing", th ? "กำลังทำ" : "Processing"],
              ["completed", th ? "เสร็จสิ้น" : "Completed"],
              ["failed", th ? "ผิดพลาด" : "Failed"],
          ...
          <span className="text-xs font-body text-muted ml-auto whitespace-nowrap">
            {th ? `${filtered.length} / ${rows.length} รายการ` : `${filtered.length} of ${rows.length}`}
          </span>
        </div>

        {rows.length === 0 ? (
          <div className="glass rounded-2xl py-20 px-8 text-center flex flex-col items-center gap-6">
            <MascotBot variant="idle" size="lg" message={th ? "ยังไม่มีการสแกนในระบบ" : "No scans yet"} />
            <p className="font-display font-bold text-ink text-xl">
              {th ? "ยังไม่มีการประเมินในระบบ" : "No assessments yet"}
            </p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="glass rounded-2xl p-16 text-center">
            <p className="font-display font-bold text-ink text-base">{th ? "ไม่พบรายการ" : "No scans found"}</p>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            <div className="overflow-x-auto">
              <div className="hidden lg:grid grid-cols-[auto_1fr_1fr_110px_120px_120px] gap-4 px-6 py-3 bg-bg2/40 border-b border-border/50 text-[11px] font-semibold text-muted uppercase tracking-wide">
                <span>#</span>
                <span>{th ? "ผู้ป่วย" : "Patient"}</span>
                <span>{th ? "แพทย์" : "Doctor"}</span>
                <span>{th ? "อายุกระดูก" : "Bone Age"}</span>
                <span>{th ? "สถานะ" : "Status"}</span>
                <span>{th ? "วันที่" : "Date"}</span>
              </div>
                ...
                <div className="divide-y divide-border/40">
                  {filtered.map((row, i) => {
                    ...
                    return (
                      <div key={row.id}>
                        {/* Desktop row */}
                        <div className="hidden lg:grid grid-cols-[auto_1fr_1fr_110px_120px_120px] gap-4 items-center px-6 py-3.5 hover:bg-primary/[0.04] transition-colors duration-150">
                          ...
                          <span className="text-sm text-ink font-medium">{boneAgeText}</span>
                        ...
                        <div className="flex lg:hidden items-center gap-3 px-4 py-3.5">
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-ink text-sm truncate">{row.childName ?? "—"}</p>
                            <p className="text-[11px] text-muted truncate">{row.doctorName ?? "—"} · {boneAgeText}</p>
                          </div>
