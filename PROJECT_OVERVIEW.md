...
> เพราะโมเดลจริงจะใช้เวลาราว ~30 วินาที ถ้าให้หมอนั่งค้างหน้าจอรอจะแย่ ระบบจึง "รับงานไว้ก่อน" แล้วให้หน้าจอคอยถามเป็นระยะว่าเสร็จหรือยัง (เรียกว่า *polling*)
> *(ตอนนี้ AI ยังเป็น mock เลยตอบแทบจะทันที — แต่โครงระบบออกแบบเผื่อโมเดลจริงไว้แล้ว ดู[หัวข้อ 9.1](#️-91-สถานะปัจจุบัน-ai-ยังเป็น-mock))*
...
**จุดสำคัญของฟีเจอร์นี้:** หมอกดส่ง **ครั้งเดียว** แต่ผู้ปกครองได้รับ **2 ช่องทาง**
...
ไม่ต้องมีคนกดเอง — Patient มี "นาฬิกาปลุก" ในตัว คอยเช็กทุก 12 ชั่วโมง
...
### ⚠️ 9.1 สถานะปัจจุบัน: AI ยังเป็น Mock

> **สิ่งสำคัญที่สุดที่คนเพิ่งเข้ามาต้องรู้:** ตอนนี้ AI service **ยังไม่ได้รันโมเดลจริง** — มันตอบผล "จำลองแบบสมจริง" กลับมาแทน

```mermaid
graph LR
  subgraph NOW["🎭 ตอนนี้ (Mock)"]
    I1["ภาพ X-ray"] --> M["สุ่มผลแบบสมจริง<br/>+ คำนวณ TW3/กราฟไทย จริง"] --> O1["ผลลัพธ์<br/>isMock = true 🏷️"]
  end
  subgraph SOON["🧠 เป้าหมาย (โมเดลจริง)"]
    I2["ภาพ X-ray"] --> R["DeepLabV3 → YOLOv11<br/>→ ConvNeXt → Grad-CAM"] --> O2["ผลลัพธ์<br/>isMock = false ✅"]
  end
```

**สิ่งที่จริงแล้ว vs ยังจำลองอยู่:**

| ส่วน | สถานะ |
|------|--------|
| โครง API + การรับ-ส่งภาพ + การคุยกับ Patient service | ✅ ของจริง ใช้งานได้ |
| สูตร TW3 ทำนายส่วนสูง + เทียบกราฟเกณฑ์เด็กไทย (`growth_standards.py`) | ✅ ของจริง คำนวณจริง |
| อายุกระดูกจากภาพ (DeepLabV3 → YOLOv11 → ConvNeXt → Grad-CAM) | 🎭 จำลอง — ไลบรารี ML (torch ฯลฯ) ยังถูกคอมเมนต์ไว้ใน `requirements.txt` |
| ภาพ heatmap | 🎭 จำลอง (ลิงก์ขึ้นต้น `mock://`) |

**ระบบป้องกันความเข้าใจผิด:** ทุกผลจำลองติดธง `isMock=true` ติดไปตลอดทาง — ฐานข้อมูล → หน้าเว็บ → รายงาน PDF ล้วนแสดงป้าย "ผลจำลอง" ให้เห็นชัด

**จะทำให้เป็นของจริงต้องทำอะไร:** เอา weights ที่เทรนบน Google Colab (`deeplabv3.pt`, `yolov11.pt`, `convnext.pt`) มาวางใน `MODEL_DIR` + เปิดคอมเมนต์ไลบรารี ML + implement `load_models()` ใน `bone_age.py` (ในไฟล์มี comment บอกขั้นตอนไว้แล้ว)
...
### 13.1 ตอนนี้เราอยู่ตรงไหน

ทุกวันนี้ระบบรันแบบ **Development** — ทุกอย่างอยู่บนคอมของเรา คนอื่นเข้าไม่ได้:

```mermaid
graph LR
  subgraph PC["💻 คอมของเรา (ตอนนี้)"]
    direction TB
    subgraph DOCKER["🐳 Docker (มีแค่ส่วนนี้)"]
      DB1[("Postgres ×2")]
      RD{{"Redis"}}
    end
    SV["Web + Gateway + Auth<br/>+ Patient + AI + Notify<br/>รันด้วย pnpm dev ตรง ๆ<br/>(ยังไม่มี Dockerfile)"]
    SV --- DOCKER
  end
  X["🌍 คนอื่นบนอินเทอร์เน็ต"] -. "❌ เข้าไม่ได้" .-> PC
```

**สังเกต:** Docker ตอนนี้ใช้แค่รัน "ฐานข้อมูล + Redis" — ตัว service ทั้ง 6 ยังรันบนเครื่องตรง ๆ และ**ยังไม่มี Dockerfile** ของแต่ละ service เลย
...
### 13.3 ด่านที่ต้องผ่านก่อนขึ้น Production (Blockers) 🚧

3 เรื่องนี้**ต้องแก้ก่อน** ไม่งั้นขึ้นไปก็ยังไม่ใช่ของจริง/ไม่ปลอดภัย:

| # | ด่าน | ตอนนี้เป็นยังไง | ต้องทำอะไร |
|---|------|-----------------|------------|
| 1 | 🎭 **AI ยังเป็น Mock** | ตอบผลจำลอง (`isMock=true`) ไลบรารี ML ยังถูกคอมเมนต์ | ใส่ weights จริงจาก Colab + implement `load_models()` (ดู[หัวข้อ 9.1](#️-91-สถานะปัจจุบัน-ai-ยังเป็น-mock)) |
| 2 | 💣 **ฐานข้อมูลใช้โหมด auto-sync** | TypeORM `synchronize` เปิดอยู่ — โค้ดเปลี่ยน = โครงสร้างตารางเปลี่ยนเองอัตโนมัติ ตอน dev สะดวก แต่บน production **อาจลบคอลัมน์/ข้อมูลจริงทิ้งโดยไม่ถาม** | ตั้ง `DB_SYNC=false` + เขียน migration (ใบสั่งแก้โครงสร้างตารางแบบตรวจสอบได้ทีละขั้น) |
| 3 | 🔓 **คลังรูปภาพเป็น public** | ใครมีลิงก์ก็เปิดภาพ X-ray ได้ — เป็นข้อมูลสุขภาพของเด็ก (อ่อนไหวตาม PDPA) | เปลี่ยน bucket เป็น **private** + ใช้ **signed URL** (ลิงก์ชั่วคราวมีวันหมดอายุ) |
...
- [ ] **CORS** — ตั้ง `WEB_ORIGIN` ของ gateway เป็นโดเมนจริงของ Vercel (ไม่ใช่ localhost)
...
```mermaid
graph LR
  S1["1️⃣ ใส่โมเดล AI จริง<br/>(ปลดธง isMock)"] --> S2["2️⃣ เขียน Dockerfile ทุก service<br/>+ ทดสอบทั้งระบบใน Docker<br/>บนเครื่องเราให้ผ่านก่อน"]
  S2 --> S3["3️⃣ เก็บความปลอดภัย<br/>migrations + secrets<br/>+ private bucket"]
  S3 --> S4["4️⃣ Deploy หลังบ้าน<br/>ขึ้น Railway"]
  S4 --> S5["5️⃣ Deploy หน้าเว็บ<br/>ขึ้น Vercel"]
  S5 --> S6["6️⃣ ทดสอบครบวงจร<br/>สมัคร→OTP→อัปโหลด X-ray<br/>→ผล AI→เมลถึงผู้ปกครอง"]
```
